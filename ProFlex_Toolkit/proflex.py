...
class ProFlex:
    ...
    def scale_rmsf(values):
        ...
        Parameters:
        values (list of float): List of RMSF values to be scaled.

        Returns:
        list of float: Scaled values between 0 and 1.
        """
    ...
    def translate_to_alphabet(self, scaled_values):
        ...
        Parameters:
        scaled_values (list of float): Scaled RMSF values (between 0 and 1).

        Returns:
        list of str: Alphabet characters representing the scaled values.
        """
    ...
    def backtranslate_to_values(self, letters):
        ...
        Parameters:
        letters (list of str): Alphabet characters representing scaled values.

        Returns:
        list of float: Scaled RMSF values corresponding to the alphabet characters.
        """
        n_bins = len(self.ALPHABET)
    ...
    def encode_sequence(self, rmsf_values):
        """
        Encode a sequence of RMSF values to an alphabet string.

        Parameters:
        rmsf_values (list of float): List of RMSF values.

        Returns:
        list of str: Encoded alphabet sequence.
        """
        scaled_values = self.scale_rmsf(rmsf_values)
        return self.translate_to_alphabet(scaled_values)

    def decode_sequence(self, alphabet_sequence):
        """
        Decode an alphabet sequence back to scaled RMSF values.

        Parameters:
        alphabet_sequence (list of str): Alphabet sequence to decode.

        Returns:
        list of float: Decoded RMSF values.
        """
        return self.backtranslate_to_values(alphabet_sequence)

    @staticmethod
    def read_fasta_like_file(file_path):
        """
        Reads a FASTA-like file and extracts sequences of RMSF values.

        The FASTA-like format is assumed to have headers starting with '>', followed by
        lines of comma-separated RMSF values.

        Parameters:
        file_path (str): Path to the FASTA-like input file.

        Returns:
        dict: Dictionary where keys are sequence headers and values are lists of RMSF values (floats).
        """
        sequences = {}
        current_header = None
    ...
    def process_multifasta(self, seqs):
        ...
        Parameters:
        seqs (dict): Dictionary where keys are sequence headers and values are lists of RMSF values.
...
if __name__ == "__main__":
    # Example usage
    directory_path = "/media/damian/PDB_DB/ProFlex_Program/SWISSPROT_PF_DB/DATABASE"
    fasta_file = "ProFlex.fasta"

    # Create an NGramDatabase instance
    db = NGramDatabase(n=5)

    # Parse the multifasta file
    db.parse_multifasta(fasta_file)

    # Create the n-gram database
    db.create_ngram_database()
    db.save_to_directory(directory_path)
